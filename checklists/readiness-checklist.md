...
- [x] **Database ready:** container DB đã chạy và phản hồi `pg_isready`. Kiểm tra bằng `docker exec -it fit4110-vision-db-lab05 pg_isready -U $POSTGRES_USER`.
- [x] **AI service ready:** container model service trả về `200` cho endpoint `/health` và `/predict` hoạt động.
- [x] **API ready:** container API trả `200` cho `/health` và có thể tạo/lấy detections khi token hợp lệ.
...
- [x] **Network & Ports:** mạng `team-internal` hoạt động; API gọi được model service bằng hostname `vision-model`; ports 8000 (API), 9000 (model) và 5432 (DB) được map đúng.
...
- Image tags: `ghcr.io/dvkncnnt1708/lab-5-ductuanvuong/ai-vision-api:v0.1.0-team-vision` và `ghcr.io/dvkncnnt1708/lab-5-ductuanvuong/vision-model:v0.1.0-team-vision`.
